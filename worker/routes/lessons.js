...
lessonRoutes.post('/lesson-plans', async (c) => {
    ...
    try {
        ...
        for (let i = 0; i < lessons.length; i++) {
            ...
            try {
                await db.prepare('INSERT INTO lessons (lesson_plan_id, lesson_order, title, description, content, uuid) VALUES (?, ?, ?, ?, ?, ?)').bind(lessonPlanId, lesson.lesson_order || i + 1, lesson.title, lesson.description || null, lesson.content || null, lesson.uuid || null).run();
...
lessonRoutes.get('/lesson-plans', async (c) => {
    ...
    try {
        ...
        const lessonPlansWithLessons = await Promise.all(lessonPlans.results.map(async (plan) => {
